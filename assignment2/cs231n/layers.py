...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_pad = np.pad(x,constant_values=0,pad_width=npad)
...
def conv_backward_naive(dout, cache):
    ...
    npad = ((0,0), (0,0), (p, p), (p, p))
    x_pad = np.pad(x,constant_values=0,pad_width=npad)
    dx,dx_pad,dw,db = np.zeros_like(x),np.zeros_like(x_pad),np.zeros_like(w),np.zeros_like(b)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(Hh):
      for j in range(Ww):
        out[:,:,i,j] = np.max(x[:,:,i*s:i*s+p_h,j*s:j*s+p_w], axis = (2,3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for f in range(C):
        for i in range(Hh):
          for j in range(Ww):
            index = np.argmax(x[n,f,i*s:i*s+p_h,j*s:j*s+p_w])
            ind1,ind2 = np.unravel_index(index,(p_h,p_w))
            dx[n,f,i*s:i*s+p_h,j*s:j*s+p_w][ind1,ind2] = dout[n,f,i,j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
