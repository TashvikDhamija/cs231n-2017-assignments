...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    z1 = np.maximum(0,X.dot(W1) + b1)    # (n,h)
    z2 = z1.dot(W2) + b2   # (n,c)
    scores = z2
    #############################################################################
    #                              END OF YOUR CODE                             #
    ...
    c_logprob = -np.log(prob[np.arange(N),y])
    data_loss = np.sum(c_logprob) / N
    reg_loss = reg*(np.sum(W2*W2)+np.sum(W1*W1))
    ...
    dhid = dscores.dot(W2.T)
    dhid[z1==0] = 0
    
    grads['W1'] = X.T.dot(dhid)
    grads['b1'] = np.sum(dhid,axis = 0,keepdims = True)
